...
class DynamicLSTMModel(nn.Module):
    def __init__(self, c_in: int, hidden_size: int, num_layers: int, output_features: int):
        super(DynamicLSTMModel, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.output_features = output_features

        # Note: c_in is not specified here because it will be dynamically determined
        # self.lstm = None  # Initialized dynamically based on input
        # self.fc = None  # Fully connected layer, re-initialized for each forward pass based on output size
        self.lstm = nn.LSTM(c_in, self.hidden_size, self.num_layers, batch_first=True)

        self.fc = nn.Linear(self.hidden_size, self.output_features)


    def forward(self, x: torch.FloatTensor) -> torch.FloatTensor:
        batch_size, num_sequences, seq_len, c_in = x.shape
        

        # Reshape x to (batch_size * num_sequences, seq_len, c_in) for LSTM
        x = x.view(batch_size * num_sequences, seq_len, c_in)
        ...
        # Reshape output to (batch_size, num_sequences, seq_len, output_features)
        out = out.view(batch_size, num_sequences, seq_len, -1)
...
class MTGNNLayer(nn.Module):
    ...
    def __init__(
        ...
        # Define the mapping from j to c_in
        j_to_cin = {
            1: 16,
            2: 15,
            3: 13,
        ...
        c_in = j_to_cin.get(j, 1)  # Default to 1 if j is not in the mapping

        self._filter_conv = DynamicLSTMModel(
            c_in = c_in,
            hidden_size=20, 
            num_layers=1,
            output_features=receptive_field - rf_size_j + 1,  # Use the determined output features

        )

        self._gate_conv = DynamicLSTMModel(
            c_in = c_in,
            hidden_size=20, 
            num_layers=1,
            output_features=receptive_field - rf_size_j + 1,  # Use the determined output features
